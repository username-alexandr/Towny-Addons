...
classes = server / 'citizens-probe-build'
classes.mkdir(exist_ok=True)
jars = sorted((server / 'versions').rglob('*.jar')) + sorted((server / 'libraries').rglob('*.jar')) + sorted((server / 'plugins').glob('*.jar'))
jars = [p for p in jars if p.name != 'NeverLandCitizensRuntimeProbe.jar']
subprocess.run([str(args.java_home / 'bin/javac'), '-encoding', 'UTF-8', '-d', str(classes),
                '-classpath', os.pathsep.join(map(str, jars)), str(source / 'CitizensRuntimeProbe.java')], check=True)
subprocess.run([str(args.java_home / 'bin/jar'), '--create', '--file', str(server / 'plugins/NeverLandCitizensRuntimeProbe.jar'),
