...
a=p.parse_args();s=a.server_dir.resolve();proof=s/'plugins/NeverLandSiegesRuntimeProbe'
if not (s/'ALLOW_DISPOSABLE_RELIABILITY_PROBE').is_file():raise SystemExit('Disposable server marker required')
if not (s/'plugins/NeverLandSiegesRuntimeProbe.jar').is_file():raise SystemExit('Compile and install the probe first')
...
(s/('sieges-'+a.phase+'-jars.json')).write_text(json.dumps(hashes,indent=2)+'\n')
with (s/('sieges-'+a.phase+'.log')).open('w') as log:
...
if result.returncode!=0 or not (proof/(a.phase+'-passed.txt')).exists():raise SystemExit('Missing successful proof; inspect sieges-'+a.phase+'.log')
