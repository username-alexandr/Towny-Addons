"""Run exactly one phase synchronously; run first and restart sequentially on the same prepared server."""
import argparse, subprocess
from pathlib import Path
import yaml, zipfile, collections, hashlib, json
p=argparse.ArgumentParser(description=__doc__)
p.add_argument('--server-dir',type=Path,required=True);p.add_argument('--java-home',type=Path,required=True);p.add_argument('--phase',choices=['first','disabled','resumed'],required=True)
a=p.parse_args();s=a.server_dir.resolve();proof=s/'plugins/NeverLandSafetyRuntimeProbe'
if not (s/'ALLOW_DISPOSABLE_RELIABILITY_PROBE').is_file():raise SystemExit('Disposable server marker required')
if not (s/'plugins/NeverLandSafetyRuntimeProbe.jar').is_file():raise SystemExit('Compile and install the probe first')
...
prior={'disabled':'first','resumed':'disabled'}.get(a.phase)
if prior and not (proof/(prior+'-passed.txt')).exists():raise SystemExit('Previous phase must pass before restart')
...
(s/('safety-'+a.phase+'-jars.json')).write_text(json.dumps(hashes,indent=2)+'\n')
with (s/('safety-'+a.phase+'.log')).open('w') as log:
...
if result.returncode!=0 or not (proof/(a.phase+'-passed.txt')).exists():raise SystemExit('Missing successful proof; inspect safety-'+a.phase+'.log')
import shutil
shutil.copy2(s/'logs/latest.log',s/('safety-'+a.phase+'-server.log'))
