...
classes = server / 'quests-probe-build'
classes.mkdir(exist_ok=True)
jars = sorted((server / 'versions').rglob('*.jar')) + sorted((server / 'libraries').rglob('*.jar')) + sorted((server / 'plugins').glob('*.jar'))
jars = [p for p in jars if p.name != 'NeverLandQuestsRuntimeProbe.jar']
jars += [p.resolve() for p in args.extra_jar]
subprocess.run([str(args.java_home / 'bin/javac'), '-encoding', 'UTF-8', '-d', str(classes),
                '-classpath', os.pathsep.join(map(str, jars)), str(source / 'QuestsRuntimeProbe.java')], check=True)
subprocess.run([str(args.java_home / 'bin/jar'), '--create', '--file', str(server / 'plugins/NeverLandQuestsRuntimeProbe.jar'),
