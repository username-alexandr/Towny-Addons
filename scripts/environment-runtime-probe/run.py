"""Run the opt-in city environment fixture in two fresh JVMs."""
import argparse
import hashlib
import json
...
proof = server/'plugins/NeverLandEnvironmentRuntimeProbe'
...
for phase in ['first', 'restart']:
    with (server/('environment-'+phase+'.log')).open('w') as log:
    ...
    if code != 0 or (proof/'failed.txt').exists() or not (proof/(phase+'-passed.txt')).is_file():
        raise SystemExit(phase+' failed; inspect '+str(server/('environment-'+phase+'.log')))
