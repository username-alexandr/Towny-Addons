...
a=p.parse_args();s=a.server_dir.resolve();proof=s/'plugins/NeverLandCrimeRuntimeProbe'
if not (s/'ALLOW_DISPOSABLE_RELIABILITY_PROBE').is_file():raise SystemExit('Disposable server marker required')
if not (s/'plugins/NeverLandCrimeRuntimeProbe.jar').is_file():raise SystemExit('Compile and install the probe first')
...
(s/('crime-'+a.phase+'-jars.json')).write_text(json.dumps(hashes,indent=2)+'\n')
with (s/('crime-'+a.phase+'.log')).open('w') as log:
...
if result.returncode!=0 or not (proof/(a.phase+'-passed.txt')).exists():raise SystemExit('Missing successful proof; inspect crime-'+a.phase+'.log')
