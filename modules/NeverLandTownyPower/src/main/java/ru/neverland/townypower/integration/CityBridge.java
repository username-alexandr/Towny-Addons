...
import org.bukkit.*;
import ru.neverland.townypower.model.PowerEngine.Building;
import java.util.*;
...
public final class CityBridge {
    public Map<String,Building> buildings(UUID town)throws ReflectiveOperationException {
        ...
        for(var entry:values.entrySet()){
            ...
            result.put(id,new Building(Math.max(0,Math.min(5,completed)),owned(town,b),ru.neverland.integration.BuildingOperations.maintained(town,id),ru.neverland.integration.DistrictBonuses.multiplier(town,id)));
        }
