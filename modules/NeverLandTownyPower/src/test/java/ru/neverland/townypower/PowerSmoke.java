...
public final class PowerSmoke {
    ...
    public static void main(String[] args)throws Exception{
        ...
        check(profiles.size()==101,"101 catalog profiles");check(profiles.values().stream().filter(PowerProfile::producer).count()==4,"four generators");check(profiles.values().stream().filter(PowerProfile::consumer).count()==20,"20 advanced consumers");
        ...
        Map<String,Building> metropolis=new HashMap<>();profiles.forEach((id,p)->metropolis.put(id,built(p.generation().size())));var large=PowerEngine.calculate(profiles,metropolis,empty);check(large.generation()==2114&&large.demand()==1010&&large.supplied()==1010,"late-game network budget");
        var exclusive=PowerEngine.calculate(profiles,Map.of("power_station",built(5),"industrial_works",new Building(5,true,true,1,false)),empty);check(exclusive.buildings().get("industrial_works").status()==Status.SPECIALIZATION_LOCKED&&exclusive.demand()==0,"inactive old specialization cannot consume power");
        ...
        System.out.println("PowerSmoke OK: 101 profiles, four sources, 20 consumers, shortage recovery, priorities, shutdown, maintenance, ownership, districts, 1000 capacity cycles and atomic persistence");
    }
}
