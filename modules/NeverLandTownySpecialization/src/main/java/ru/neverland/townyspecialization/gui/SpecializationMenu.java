...
public final class SpecializationMenu implements Listener{
    ...
    public void open(Player p,UUID town,String id){if(!access(p,town)){tell(p,"Нет доступа к этому городу.");return;}var s=service.specialization(town).orElse(null);if(s==null){tell(p,"Ожидается расчёт города.");return;}var settings=service.settings();var v=new View(town);v.inventory=ru.neverland.core.MenuStyle.inventory(plugin, v,54,color("&6Специализация города"));
    ...
    private void button(View v,int slot,Material material,String name,List<String> lore,Runnable action){var item=new ItemStack(material);var meta=item.getItemMeta();meta.setDisplayName(ru.neverland.core.MenuStyle.nameLegacy(color(name)));meta.setLore(ru.neverland.core.MenuStyle.loreStrings(lore.stream().map(s->color("&7"+s)).toList()));item.setItemMeta(meta);v.inventory.setItem(slot,item);if(action!=null)v.actions.put(slot,action);}
