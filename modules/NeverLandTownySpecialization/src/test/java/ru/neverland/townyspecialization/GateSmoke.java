...
public final class GateSmoke {
    ...
    public static void run()throws Exception{
        Map<String,Plugin> plugins=new HashMap<>();var services=new SimpleServicesManager();var manager=proxy(PluginManager.class,(p,m,a)->m.getName().equals("getPlugin")?plugins.get(a[0]):fallback(p,m,a));var server=proxy(Server.class,(p,m,a)->switch(m.getName()){case "getPluginManager"->manager;case "getServicesManager"->services;default->fallback(p,m,a);});var field=Bukkit.class.getDeclaredField("server");field.setAccessible(true);Object previous=field.get(null);field.set(null,server);
        try{UUID town=UUID.randomUUID(),other=UUID.randomUUID();check(BuildingOperations.active(town,"foundry"),"old ordinary buildings remain optional");for(String id:SpecializationRules.PROJECTS.keySet())check(!BuildingOperations.active(town,id),"unique buildings require specialization plugin");
            ...
            var api=proxy(TownySpecializationApi.class,(p,m,a)->{if(broken.get())throw new IllegalStateException("provider failed");return switch(m.getName()){case "canUseBuilding"->town.equals(a[0])&&chosen.get().equals(SpecializationRules.required((String)a[1]));case "bonus","productionBonus"->town.equals(a[0])?value.get():0d;default->fallback(p,m,a);};});services.register(TownySpecializationApi.class,api,plugin,ServicePriority.Normal);
