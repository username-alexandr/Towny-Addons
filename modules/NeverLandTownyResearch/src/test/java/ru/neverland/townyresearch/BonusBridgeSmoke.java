...
public final class BonusBridgeSmoke {
    ...
    public static void run()throws Exception{
        ...
        var server=proxy(Server.class,(p,m,a)->switch(m.getName()){case "getPluginManager"->manager;case "getServicesManager"->services;default->fallback(p,m,a);});var field=Bukkit.class.getDeclaredField("server");field.setAccessible(true);Object previous=field.get(null);field.set(null,server);
        try{UUID town=UUID.randomUUID();check(ResearchBonuses.production(town,"foundry")==1,"optional research absent is neutral");var enabled=new AtomicBoolean(true);var broken=new AtomicBoolean(false);var value=new AtomicReference<Double>(.2);var plugin=proxy(Plugin.class,(p,m,a)->m.getName().equals("isEnabled")?enabled.get():fallback(p,m,a));plugins.put("NeverLandTownyResearch",plugin);
            var api=proxy(TownyResearchApi.class,(p,m,a)->{if(m.getName().equals("bonus")){if(broken.get())throw new IllegalStateException("provider failed");check(a[0].equals(town)&&a[1].equals("metallurgy"),"correct city and technology");return value.get();}return fallback(p,m,a);});services.register(TownyResearchApi.class,api,plugin,ServicePriority.Normal);
