...
public final class ResearchService implements TownyResearchApi {
    ...
    public void refresh(int seconds){primary();Map<UUID,ResearchSnapshot> towns=new HashMap<>();Map<UUID,UUID> residents=new HashMap<>();
        try{for(Town town:List.copyOf(TownyAPI.getInstance().getTowns())){
            ...
            try{processor.tick(id,seconds,ready,ru.neverland.integration.SpecializationAccess.bonus(id,"research_speed"));var stock=bridge.knowledge(id);knowledge=stock.balance();reserve=stock.reserve();}catch(Exception ex){paused=true;status="Операция ожидает восстановления; проверьте консоль";warn(ex);}
