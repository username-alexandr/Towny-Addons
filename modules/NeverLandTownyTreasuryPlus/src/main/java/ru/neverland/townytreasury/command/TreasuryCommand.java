...
public final class TreasuryCommand implements TabExecutor {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] raw){String[] args=clean(raw);try{
        ...
        switch(action){case "menu"->menu.open(player,id,null);case "enable","disable","shares","transfer"->menu.prepare(player,id,action,List.of(Arrays.copyOfRange(args,1,args.length)));case "confirm"->{service.confirm(player);tell(player,"Бюджет обновлён.");menu.open(player,id,null);}case "history","reports"->menu.open(player,id,action);case "report"->{String week=args.length>1?args[1]:Week.previous(System.currentTimeMillis());menu.open(player,id,"report/"+week);}case "export"->{if(!TreasuryAccess.canExport(player))throw new IllegalArgumentException("Нет права на экспорт CSV");String week=args.length>1?args[1]:Week.previous(System.currentTimeMillis());menu.export(player,id,week);}default->tell(player,"/t treasury [enable|disable|transfer|shares|confirm|reports|report|history"+(TreasuryAccess.canExport(player)?"|export":"")+"]");}
    }catch(Exception ex){tell(sender,"&c"+ex.getMessage());}return true;}
    @Override public List<String> onTabComplete(CommandSender sender,Command command,String alias,String[] raw){var args=clean(raw);List<String> options=args.length<=1?List.of("enable","disable","transfer","shares","confirm","reports","report","export","history"):args[0].equalsIgnoreCase("transfer")&&args.length<=3?List.of("construction","army","infrastructure","social","free"):List.of();String prefix=args.length==0?"":args[args.length-1].toLowerCase(Locale.ROOT);return options.stream().filter(s->!s.equals("export")||TreasuryAccess.canExport(sender)).filter(s->s.startsWith(prefix)).toList();}
}
