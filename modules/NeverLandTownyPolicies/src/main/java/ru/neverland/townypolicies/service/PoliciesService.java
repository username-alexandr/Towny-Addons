...
public final class PoliciesService implements TownyPoliciesApi {
    ...
    private boolean taxConfigured(UUID town){try{return Boolean.TRUE.equals(ru.neverland.core.ApiServices.call("NeverLandTownyTaxes","ru.neverland.townytaxes.api.NeverLandTownyTaxesApi","municipalTaxConfigured",new Class<?>[]{UUID.class},town));}catch(Exception|LinkageError e){return false;}}
    private boolean unlocked(UUID town,String unlock){try{return Boolean.TRUE.equals(ru.neverland.core.ApiServices.call("NeverLandTownyQuests","ru.neverland.townyquests.api.TownyQuestsApi","unlocked",new Class<?>[]{UUID.class,String.class},town,unlock));}catch(Exception|LinkageError ex){return false;}}
    public String unavailable(UUID town,PolicyGroup group,PolicyOption option){if(option==null)return "Сохранённый режим отсутствует в настройках";if(option.id().equals(group.standard()))return "";if(!group.enabled())return "Политика отключена в настройках";for(String name:option.requires()){var dependency=Bukkit.getPluginManager().getPlugin(name);if(dependency==null||!dependency.isEnabled())return "Не готов модуль "+name;String minimum=DependencyVersions.MINIMUM.get(name);if(!DependencyVersions.atLeast(dependency.getDescription().getVersion(),minimum))return "Нужен "+name+" версии "+minimum+" или новее";}if(!option.unlock().isEmpty()&&!unlocked(town,option.unlock()))return "Завершите городской проект в /t quests: "+option.unlock();if(group.id().equals("taxes")&&!taxConfigured(town))return "Нет действующего городского сбора Taxes в пользу своей казны";return "";}
