...
public final class ContractMenuManager implements Listener {
    ...
    private List<String> summary(ActiveContract c){List<String> lore=new ArrayList<>(terms(c.snapshot(),c.area()));lore.add("&7Прогресс: &a"+c.progress()+"/"+c.goal());lore.add("&7Осталось: &f"+(c.timer().paused()?"Пауза • ":"")+TimeUtil.format(c.timer().remaining(System.currentTimeMillis())/1000));lore.add("&7Исполнитель: &f"+contracts.companyName(c));lore.add("&7ID: &f"+c.shortId());if(!c.funded())lore.add("&eОжидает подтверждения резерва казны");if(contracts.repository().deliveryBusy(c.id()))lore.add("&eОжидает завершения поставки");if(c.settlementStatus()!=null)lore.add("&eОжидает завершения расчёта");return lore;}
    ...
    private ItemStack item(Material m,String title,List<String> lore){ItemStack s=new ItemStack(m.isItem()&&!m.isAir()?m:Material.PAPER);ItemMeta meta=s.getItemMeta();meta.setDisplayName(ru.neverland.core.MenuStyle.nameLegacy(ColorUtil.color(title)));meta.setLore(ru.neverland.core.MenuStyle.loreStrings(lore.stream().map(ColorUtil::color).toList()));meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES);s.setItemMeta(meta);return s;}
