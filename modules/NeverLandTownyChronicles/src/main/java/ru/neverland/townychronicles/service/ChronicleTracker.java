...
public final class ChronicleTracker {
    private final JavaPlugin plugin;private final TownyHook towny;private final BuildBridge builds;private final ChronicleRepository repository;private final ChronicleService chronicles;private final DecimalFormat money=new DecimalFormat("#,##0.##");private BukkitTask task;private boolean initialSyncPending=true;
    ...
    public void start(){stop();initialSyncPending=true;long delay=Math.max(1,plugin.getConfig().getLong("tracking.first-scan-delay-seconds",3))*20L,period=Math.max(10,plugin.getConfig().getLong("tracking.scan-interval-seconds",60))*20L;task=Bukkit.getScheduler().runTaskTimer(plugin,()->scanAll(false),delay,period);}
    public void stop(){if(task!=null){task.cancel();task=null;}}
    public int scanAll(boolean manual){boolean initialSync=initialSyncPending;Set<UUID>seen=new HashSet<>();for(Town town:towny.towns()){seen.add(town.getUUID());process(town,false,initialSync);}initialSyncPending=false;for(TownChronicleState state:repository.states())if(state.active()&&!seen.contains(state.townId()))dissolve(state.townId(),state.name(),"Город отсутствует в базе Towny");repository.saveIfDirty();return seen.size();}
    public void process(Town town,boolean newEvent){process(town,newEvent,false);}
    private void process(Town town,boolean newEvent,boolean initialSync){
        ...
        state.name(town.getName());state.residents(town.getResidents().size());state.blocks(town.getTownBlocks().size());state.balance(towny.balance(town));state.lastSeen(System.currentTimeMillis());state.active(!town.isRuined());achievements(town,state,fresh,initialSync);wonders(town,state,fresh,initialSync);repository.markDirty();
    }
    ...
    private void achievements(Town town,TownChronicleState state,boolean fresh,boolean initialSync){for(int value:plugin.getConfig().getIntegerList("achievements.population"))if(town.getResidents().size()>=value)award(town,state,"population-"+value,"Население достигло "+value+" жителей",List.of("Город продолжает расти."),System.currentTimeMillis(),fresh,initialSync);for(int value:plugin.getConfig().getIntegerList("achievements.territory"))if(town.getTownBlocks().size()>=value)award(town,state,"territory-"+value,"Освоено "+value+" городских участков",List.of("Территория города расширилась."),System.currentTimeMillis(),fresh,initialSync);for(double value:plugin.getConfig().getDoubleList("achievements.treasury"))if(towny.balance(town)>=value)award(town,state,"treasury-"+(long)value,"Казна достигла "+money.format(value)+" монет",List.of("Финансовое достижение города."),System.currentTimeMillis(),fresh,initialSync);long age=TimeUtil.days(state.foundedAt());for(int days:plugin.getConfig().getIntegerList("achievements.age-days"))if(age>=days)award(town,state,"age-"+days,"Городу исполнилось "+days+" дней",List.of("Историческая веха города."),state.foundedAt()+days*86_400_000L,fresh,initialSync);}
    private void award(Town town,TownChronicleState state,String id,String title,List<String>details,long timestamp,boolean fresh,boolean initialSync){if(!state.achievements().add(id))return;chronicles.record(town.getUUID(),town.getName(),ChronicleCategory.ACHIEVEMENT,timestamp,title,details,"","NeverLandTownyChronicles",StartupAnnouncementPolicy.shouldAnnounce(fresh,initialSync));}
    private void wonders(Town town,TownChronicleState state,boolean fresh,boolean initialSync){for(Map.Entry<String,String>entry:builds.projects().entrySet()){int level=builds.level(town.getUUID(),entry.getKey()),old=state.wonders().getOrDefault(entry.getKey(),0);boolean discovered=level>0&&old<=0;if(StartupAnnouncementPolicy.shouldRecordWonder(discovered,initialSync,plugin.getConfig().getBoolean("tracking.record-existing-wonders",true)))chronicles.record(town.getUUID(),town.getName(),ChronicleCategory.WONDER,System.currentTimeMillis(),"Возведено Чудо Света — "+entry.getValue(),List.of("Отныне "+entry.getValue()+" является частью истории города."),"","NeverLandTownyBuilds",StartupAnnouncementPolicy.shouldAnnounce(fresh,initialSync));state.wonders().put(entry.getKey(),level);}}
