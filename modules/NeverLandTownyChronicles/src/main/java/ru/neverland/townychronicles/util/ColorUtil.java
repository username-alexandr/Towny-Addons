...
public final class ColorUtil {
    ...
    public static String color(String input){if(input==null)return "";Matcher matcher=HEX.matcher(input);StringBuffer out=new StringBuffer();while(matcher.find()){StringBuilder legacy=new StringBuilder("&x");for(char value:matcher.group(1).toCharArray())legacy.append('&').append(value);matcher.appendReplacement(out,Matcher.quoteReplacement(legacy.toString()));}matcher.appendTail(out);return ChatColor.translateAlternateColorCodes('&',out.toString());}
