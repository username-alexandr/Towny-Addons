...
public final class ChronicleService implements TownyChroniclesApi {
    ...
    @Override public ChronicleEntry record(UUID townId,ChronicleCategory category,String title,List<String>details,String actor,String source){ru.neverland.core.ApiServices.primaryThread();return record(townId,category,System.currentTimeMillis(),title,details,actor,source);}
    @Override public ChronicleEntry record(UUID townId,ChronicleCategory category,long timestamp,String title,List<String>details,String actor,String source){ru.neverland.core.ApiServices.primaryThread();Town town=towny.town(townId);String name=town==null?repository.state(townId).name():town.getName();return record(townId,name,category,timestamp,title,details,actor,source,true);}
    public ChronicleEntry record(UUID townId,String townName,ChronicleCategory category,long timestamp,String title,List<String>details,String actor,String source,boolean announce){ru.neverland.core.ApiServices.primaryThread();ChronicleEntry entry=new ChronicleEntry(UUID.randomUUID(),townId,townName,category,timestamp,title,details,actor,source);repository.add(entry);Bukkit.getPluginManager().callEvent(new ChronicleRecordEvent(entry));if(announce)announce(entry);return entry;}
    ...
    @Override public List<ChronicleEntry>entries(UUID townId){ru.neverland.core.ApiServices.primaryThread();return repository.entries(townId);}@Override public ChronicleEntry latest(UUID townId){ru.neverland.core.ApiServices.primaryThread();return repository.latest(townId);}public ChronicleRepository repository(){return repository;}
}
