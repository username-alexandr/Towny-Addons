...
public final class TownTaxesCommand implements CommandExecutor {
    ...
    private void status(Player p,Town t){p.sendMessage(ColorUtil.color("&#FFD45A&lКазначейство города "+t.getName()));p.sendMessage(ColorUtil.color("&7Баланс города: &f"+fiscal.format(t.getAccount().getHoldingBalance())));Resident r=towny.resident(p);p.sendMessage(ColorUtil.color("&7Ваша задолженность: &f"+fiscal.format(r==null?0:fiscal.taxDebt(r.getUUID()))));p.sendMessage(ColorUtil.color("&7Городских сборов: &f"+fiscal.repository().policies().stream().filter(v->v.scope()==Domain.Scope.TOWN&&v.targetId().equals(t.getUUID())).count()));messages.list("help-taxes").forEach(p::sendMessage);}
    private void list(Player p,Town t){p.sendMessage(ColorUtil.color("&#FFD45A&lГородские налоговые сборы:"));boolean any=false;for(TaxPolicy v:fiscal.repository().policies())if(v.scope()==Domain.Scope.TOWN&&v.targetId().equals(t.getUUID())){any=true;p.sendMessage(ColorUtil.color("&8- &f"+v.shortId()+" &7"+taxName(v.type())+": &e"+v.value()+(v.type()==Domain.TaxType.FIXED?" монет":"%")+" &8→ &f"+v.destinationName()+(ru.neverland.townytaxes.model.MunicipalTaxes.matches(v,t.getUUID())?" &7(по указу: &e"+fiscal.format(v.value()*ru.neverland.integration.PoliciesAccess.tax(t.getUUID()))+(v.type()==Domain.TaxType.FIXED?" монет":"%")+"&7; до санкций и соглашений)":"")));}if(!any)p.sendMessage(ColorUtil.color("&7Сборы не настроены."));}
    private void create(Player p,Town t,String[] a){if(!p.hasPermission("townytaxes.manage")||!towny.isTownManager(p,t,"tax")){messages.send(p,"only-town-manager");return;}if(a.length<3){p.sendMessage(ColorUtil.color("&fИспользование: /t taxes create <fixed|income|transaction> <значение> [интервал в минутах]"));return;}Domain.TaxType type=Domain.TaxType.parse(a[1]);if(type==null){messages.send(p,"invalid-tax-type");return;}try{double value=Double.parseDouble(a[2].replace(',','.'));if(!fiscal.validTaxValue(type,value))throw new NumberFormatException();long minutes=a.length>3?Long.parseLong(a[3]):1440;TownyHook.Party party=new TownyHook.Party(Domain.Scope.TOWN,t.getUUID(),t.getName(),t.getAccount());TaxPolicy policy=fiscal.createPolicy(party,type,value,party,fiscal.intervalMillis(minutes),p.getName());messages.send(p,"policy-created",Map.of("id",policy.shortId()));}catch(NumberFormatException e){messages.send(p,"invalid-number");}}
    private void remove(Player p,Town t,String[] a){if(!p.hasPermission("townytaxes.manage")||!towny.isTownManager(p,t,"tax")){messages.send(p,"only-town-manager");return;}if(a.length<2){p.sendMessage("/t taxes remove <ID>");return;}TaxPolicy v=fiscal.repository().policy(a[1]);if(v==null||v.scope()!=Domain.Scope.TOWN||!v.targetId().equals(t.getUUID())){messages.send(p,"policy-not-found",Map.of("id",a[1]));return;}fiscal.removePolicy(a[1]);messages.send(p,"policy-removed",Map.of("id",v.shortId()));}
